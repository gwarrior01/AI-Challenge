...
Any of those same conversations can also turn on **context management (summarization)** — an opt-in checkbox (web UI) or `--compress`/wizard step (CLI/TUI), off by default, so nothing changes unless you ask for it. With it on, only the last few exchanges are sent to the model as-is; everything older is periodically folded into a running text summary — produced by the same model, via a dedicated request — which is substituted for the full history from then on. The summary is recomputed in batches of `LLM_CONTEXT_SUMMARY_CHUNK` user messages (10 by default, see below): after every Nth message since the last recompute, the whole unsummarized tail is condensed again in one shot, rather than resummarizing after every single message. Interfaces surface this next to the context-window indicator: a notice the moment a summary is (re)computed, and a live counter of how many user messages remain until the next one — both, along with the batch size shown in the web UI's checkbox labels, always reflect the current `LLM_CONTEXT_SUMMARY_CHUNK` value. For named agents the summary is persisted to SQLite (`AGENTS_STORE_PATH`) alongside their history, so it survives restarts; for the plain web chat (which has no server-side state at all) the summary lives in the browser tab and the server only performs the one-off summarization request (`POST /api/summarize`).
...
- `n` first asks which creation mode to use: **быстро** (`1`/`q`) asks only for a name and a system prompt — every other setting (model, limits, temperature, top_p, reasoning, show-tokens, context management) is left at its default; **расширенно** (`2`/`a`) walks through the full set of fields one at a time, including a final "Сжимать контекст диалога в сводку? y/n" step (context management, see above).
...
core/             — llm-core: LLM client (src/lib.rs) + agent entity and registry (src/agent.rs) + context summarization (src/context.rs)
